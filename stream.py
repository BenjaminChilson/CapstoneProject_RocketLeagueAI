...
subprocess.call("ffmpeg -f gdigrab -s %s -framerate %s -i desktop -c:v libx264 -preset fast -pix_fmt yuv420p -s %s -threads 0 -f flv \"%s/%s\"" % (screen_resolution, target_frame_rate, stream_resolution, ingestion_server, stream_key), shell=True)